...
### 2. 引入js文件
   在需要使用Bainu js-sdk的页面引入如下js文件。`http://static.zuga-tech.net/open/bainu-js-sdk-1.4.js`
...
#### 分享到bainu好友
```javascript
bainu.onMenuShareChat({
  'title': '', //分享标题，长度不能超过300，默认为网页标题
  'url': '', //链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致，默认为当前网页链接
  'desc': '', //分享描述，长度不能超过500，可空
  'iconURL': '', //分享icon，可空
  success: function () {
    // 用户确认分享后执行的回调函数
  },
...
#### 分享到bainu朋友圈
```javascript
bainu.onMenuShareMoment({
  'title': '', //分享标题，长度不能超过300，默认为网页标题
  'url': '', //分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致，默认为当前网页链接
  'iconURL': '', //分享icon，可空
  success: function () {
    // 用户确认分享后执行的回调函数
  },
...
bainu.uploadImage({
  ...
  'success': function(res){
    var urls = res.urls; // 返回图片localId对应的网络地址 {localId:url,...}
   },
});
```
...
bainu.uploadVideo({
  ...
  'success' : function(res) {
    var url = res.url, // 视频地址
        ...
        image = res.image, // 视频截图地址
        width = res.width, // 宽度（像素）
...
```javascript
bainu.getLocation({
  'latitude': 0, // 默认纬度，浮点数，范围为90 ~ -90
  'longitude': 0// 默认经度，浮点数，范围为180 ~ -180
...
### 附录
#### 1.菜单列表项
- 分享到bainu聊天:menu.share.chat
- 分享到chomorlig:menu.share.moment
- 分享到微信聊天:menu.share.wechat
- 分享到微信朋友圈:menu.share.wechatMoment
- 添加到收藏:menu.favorite
